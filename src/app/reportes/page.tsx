...
export default function ReportesPage() {
  ...
  const [selectedChurch, setSelectedChurch] = useState<string>('ALL'); // Por defecto en Global
  ...
  // Función para marcar/desmarcar entrega en vivo
  const confirmToggleDelivery = async () => {
    if (!deliveryModalFamily) return;
    
    const { id, is_delivered } = deliveryModalFamily;

    // 1. Ocultar el modal instantáneamente para que se sienta rápido
    setDeliveryModalFamily(null);

    // 2. Actualización visual instantánea en la tabla
    setReportData((prev) => {
      if (!prev) return prev;
      return {
        ...prev,
    ...
    if (!res.success) {
      alert('Error de conexión al guardar la entrega.');
    }
  };
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <header className="bg-gray-800 p-6 rounded-2xl border border-gray-700 flex flex-col md:flex-row justify-between items-center gap-4 print:hidden">
          <div>
            ...
            <p className="text-sm text-gray-400">Consolidado exacto de producción y listas de distribución</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
            <div>
              ...
              >
                <option value="ALL">🌍 Todas las Iglesias (Global)</option>
        ...
        {loading || !reportData ? (
          ...
          <main className="bg-gray-800 p-8 rounded-2xl border border-gray-700 print:bg-white print:border-none print:shadow-none space-y-12">
            ...
            {/* ----------------------------------------------------------- */}
            {/* SECCIÓN 2: LISTA DE DISTRIBUCIÓN POR FAMILIA (Día de Entrega)*/}
            {/* print:break-before-page fuerza a que esto se imprima en una hoja nueva */}
            {/* ----------------------------------------------------------- */}
            <div className="print:break-before-page pt-8">
              <div className="border-b border-gray-700 print:border-gray-300 pb-4 mb-6">
                ...
                <p className="text-xs text-gray-400 print:text-gray-600">Verifica el saldo y marca la casilla al entregar las prendas físicas a cada familia.</p>
              </div>
                    ...
                    <div key={family.id} className="border border-gray-700 print:border-gray-300 rounded-xl p-4 flex flex-col justify-between break-inside-avoid">
                      ...
                      <div className="flex justify-between items-start mb-3 border-b border-gray-700 print:border-gray-200 pb-2">
                        ...
                        {family.is_fully_paid ? (
                          <span className="bg-emerald-500/20 print:bg-transparent text-emerald-400 print:text-black border border-emerald-500/30 print:border-emerald-700 px-2 py-1 rounded text-xs font-bold">
                            ✓ PAGADO
                          </span>
                        ) : (
                          <div className="text-right">
        ...
        {/* -------------------------------------------------------------------------- */}
      {/* MODAL DE CONFIRMACIÓN DE ENTREGA / DESHACER ENTREGA                        */}
      {/* -------------------------------------------------------------------------- */}
      {deliveryModalFamily && (
